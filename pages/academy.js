...
import { SUBJECTS, LEVEL_THRESHOLDS, BADGES } from '../data/curriculum'
import FlashcardSystem from '../components/FlashcardSystem'
import { FLASHCARDS_FIN } from '../data/flashcardsFIN'
import { LEARN_CONTENT } from '../data/learnContent'
import { LEARN_CONTENT_2 } from '../data/learnContent2'
import { FIN_LEARN_INV } from '../data/finLearnINV'
import { FIN_LEARN_TAX } from '../data/finLearnTAX'
import { FIN_LEARN_CRED } from '../data/finLearnCRED'
import { FIN_LEARN_BANK } from '../data/finLearnBANK'
import { FIN_LEARN_INS } from '../data/finLearnINS'
    ...
    <div style={{ maxWidth:900, margin:'0 auto' }}>
      ...
      <div style={{ marginTop:32 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
          ...
          <span style={{ background:'#A78BFA25', color:'#A78BFA', fontSize:10, fontWeight:800, padding:'2px 10px', borderRadius:20 }}>SPACED REPETITION</span>
        </div>
        <button onClick={() => setScreen('flashcards')}
          style={{ width:'100%', background:`linear-gradient(135deg, #A78BFA15, #A78BFA07)`, border:`1px solid #A78BFA40`, borderRadius:16, padding:'20px 24px', cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:16, textAlign:'left', transition:'all 0.2s' }}
          onMouseEnter={e=>{e.currentTarget.style.borderColor=`#A78BFA90`; e.currentTarget.style.transform='translateY(-1px)'}}
          onMouseLeave={e=>{e.currentTarget.style.borderColor=`#A78BFA40`; e.currentTarget.style.transform='translateY(0)'}}>
          <div style={{ width:54, height:54, borderRadius:'50%', background:`linear-gradient(135deg, #A78BFA, #A78BFA88)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:26, flexShrink:0, boxShadow:`0 4px 16px #A78BFA40` }}>🃏</div>
          <div style={{ flex:1 }}>
            <div style={{ fontFamily:"'Sora',sans-serif", fontSize:16, fontWeight:800, color:C.text, marginBottom:5 }}>Study with Flashcards</div>
            <div style={{ fontSize:13, color:C.sub, lineHeight:1.6 }}>160+ cards across Investing, Taxes, Credit, Banking & Insurance.</div>
          </div>
          <div style={{ fontSize:24, color:'#A78BFA', flexShrink:0 }}>→</div>
        </button>
      </div>
    </div>
...
const ALL_LEARN = { ...LEARN_CONTENT, ...LEARN_CONTENT_2, ...FIN_LEARN_INV, ...FIN_LEARN_TAX, ...FIN_LEARN_CRED, ...FIN_LEARN_BANK, ...FIN_LEARN_INS }
...
export default function Academy() {
  ...
  const [themeName, setThemeName] = useState(() => {
    if (typeof window !== 'undefined') {
      const s = localStorage.getItem('fiq_theme_fin')
  ...
  const toggleTheme = () => {
    ...
    setThemeName(next)
    if (typeof window !== 'undefined') localStorage.setItem('fiq_theme_fin', next)
  }
  ...
  return (
    ...
    <>
      <Head>
        <title>Financial Academy — FintelligenceIQ</title>
      ...
      <div style={{ minHeight:'100vh', background:CT.bg, position:'relative' }}>
        ...
        <div style={{ position:'fixed', inset:0, zIndex:0, pointerEvents:'none' }}>
          <img
            src="https://images.unsplash.com/photo-1524169358666-79f22534bc6e?auto=format&fit=crop&w=1920&q=25"
            alt=""
          ...
          <div className="academy-header-inner" style={{ maxWidth:980, margin:'0 auto', height:58, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <div style={{ display:'flex', alignItems:'center', gap:16 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                ...
                <span style={{ fontFamily:"'Sora',sans-serif", fontWeight:800, fontSize:15 }}>Financial Academy</span>
              </div>
            ...
            <button onClick={() => setScreen(screen === 'flashcards' ? 'home' : 'flashcards')}
              style={{ display:'flex', alignItems:'center', gap:6, background: screen === 'flashcards' ? `#A78BFA25` : 'rgba(255,255,255,0.06)', border:`1px solid ${screen === 'flashcards' ? '#A78BFA' : 'rgba(255,255,255,0.12)'}`, borderRadius:20, padding:'5px 14px', cursor:'pointer', fontFamily:'inherit', fontSize:12, fontWeight:700, color: screen === 'flashcards' ? '#A78BFA' : CT.sub, transition:'all 0.15s' }}>
        ...
        <div className="academy-content" style={{ padding:'32px 24px', maxWidth:980, margin:'0 auto', position:'relative', zIndex:1 }}>
          ...
          {screen === 'flashcards' && <FlashcardSystem onBack={() => setScreen('home')} cards={FLASHCARDS_FIN} storageKey="fiq_fc_fin" accentColor="#A78BFA" academyName="Financial Academy" />}
        </div>
      </div>
    </>
    </ThemeCtx.Provider>
