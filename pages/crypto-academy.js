...
import { FLASHCARDS_CRYPTO } from '../data/flashcardsCRYPTO'
...
import { CRYPTO_SUBJECTS as SUBJECTS, CRYPTO_LEVEL_THRESHOLDS as LEVEL_THRESHOLDS, CRYPTO_BADGES as BADGES } from '../data/cryptoCurriculum'
import { CRYPTO_LEARN as LEARN_CONTENT } from '../data/cryptoLearnContent'
import { TRADING_CHARTS, CHART_CAPTIONS } from '../data/tradingCharts'
import { CRYPTO_LEARN_2 as LEARN_CONTENT_2 } from '../data/cryptoLearnContent2'
import { CRYPTO_LEARN_CF_A } from '../data/cryptoLearnCF_A'
import { CRYPTO_LEARN_CF_B } from '../data/cryptoLearnCF_B'
import { CRYPTO_LEARN_OTHER_A } from '../data/cryptoLearnOther_A'
import { CRYPTO_LEARN_OTHER_B } from '../data/cryptoLearnOther_B'
...
const SAVE_KEY = 'fiq_crypto_academy_v1'
    ...
    <div style={{ maxWidth:900, margin:'0 auto' }}>
      ...
      <div style={{ marginTop:32 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
          ...
          <span style={{ background:'#F7931A25', color:'#F7931A', fontSize:10, fontWeight:800, padding:'2px 10px', borderRadius:20 }}>SPACED REPETITION</span>
        </div>
        <button onClick={() => setScreen('flashcards')}
          style={{ width:'100%', background:`linear-gradient(135deg, #F7931A15, #F7931A07)`, border:`1px solid #F7931A40`, borderRadius:16, padding:'20px 24px', cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:16, textAlign:'left', transition:'all 0.2s' }}
          onMouseEnter={e=>{e.currentTarget.style.borderColor=`#F7931A90`; e.currentTarget.style.transform='translateY(-1px)'}}
          onMouseLeave={e=>{e.currentTarget.style.borderColor=`#F7931A40`; e.currentTarget.style.transform='translateY(0)'}}>
          <div style={{ width:54, height:54, borderRadius:'50%', background:`linear-gradient(135deg, #F7931A, #F7931A88)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:26, flexShrink:0, boxShadow:`0 4px 16px #F7931A40` }}>🃏</div>
          <div style={{ flex:1 }}>
            <div style={{ fontFamily:"'Sora',sans-serif", fontSize:16, fontWeight:800, color:C.text, marginBottom:5 }}>Study with Flashcards</div>
            <div style={{ fontSize:13, color:C.sub, lineHeight:1.6 }}>90+ cards covering Bitcoin, Ethereum, DeFi, NFTs, Security & more.</div>
          </div>
          <div style={{ fontSize:24, color:'#F7931A', flexShrink:0 }}>→</div>
        </button>
      </div>
    </div>
...
function SubjectScreen({ subjectId, gs, setScreen, setCurrentLesson, setCurrentUnit, setCurrentLearnUnit }) {
  ...
  const isUnitUnlocked = (unitIndex) => { return true; // All content unlocked
  }
...
const ALL_LEARN = { ...LEARN_CONTENT, ...LEARN_CONTENT_2, ...CRYPTO_LEARN_CF_A, ...CRYPTO_LEARN_CF_B, ...CRYPTO_LEARN_OTHER_A, ...CRYPTO_LEARN_OTHER_B }
          ...
          <>
            ...
            {(content.sections || []).map((section, si) => (
              <div key={si} style={{ marginBottom:40 }}>
                ...
                {/* Named chart from shared chart library */}
...
export default function Academy() {
  ...
  const [themeName, setThemeName] = useState(() => {
    if (typeof window !== 'undefined') {
      const s = localStorage.getItem('fiq_theme_crypto')
  ...
  const toggleTheme = () => {
    ...
    setThemeName(next)
    if (typeof window !== 'undefined') localStorage.setItem('fiq_theme_crypto', next)
  }
  ...
  return (
    ...
    <>
      <Head>
        <title>Crypto Academy — FintelligenceIQ</title>
      ...
      <div style={{ minHeight:'100vh', background:CT.bg, position:'relative' }}>
        ...
        <div style={{ position:'fixed', inset:0, zIndex:0, pointerEvents:'none' }}>
          <img
            src="https://images.unsplash.com/photo-1639762681485-074b7f938ba0?auto=format&fit=crop&w=1920&q=25"
            alt=""
          ...
          <div className="academy-header-inner" style={{ maxWidth:980, margin:'0 auto', height:58, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <div style={{ display:'flex', alignItems:'center', gap:16 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                ...
                <span style={{ fontFamily:"'Sora',sans-serif", fontWeight:800, fontSize:15 }}>Crypto Academy</span>
              </div>
            ...
            <button onClick={() => setScreen(screen === 'flashcards' ? 'home' : 'flashcards')}
              style={{ display:'flex', alignItems:'center', gap:6, background: screen === 'flashcards' ? `#F7931A25` : 'rgba(255,255,255,0.06)', border:`1px solid ${screen === 'flashcards' ? '#F7931A' : 'rgba(255,255,255,0.12)'}`, borderRadius:20, padding:'5px 14px', cursor:'pointer', fontFamily:'inherit', fontSize:12, fontWeight:700, color: screen === 'flashcards' ? '#F7931A' : CT.sub, transition:'all 0.15s' }}>
        ...
        <div className="academy-content" style={{ padding:'32px 24px', maxWidth:980, margin:'0 auto', position:'relative', zIndex:1 }}>
          ...
          {screen === 'flashcards' && <FlashcardSystem cards={FLASHCARDS_CRYPTO} storageKey="fiq_fc_crypto" accentColor="#F7931A" academyName="Crypto Academy" onBack={() => setScreen('home')} cards={FLASHCARDS_CRYPTO} storageKey="fiq_fc_crypto" accentColor="#F7931A" academyName="Crypto Academy" />}
